...
import { BookOpen, FileKey, Play } from "lucide-react";
...
export default function DocsCollectorPage({
  ...
  params: Promise<{ provider: string; collector: string }>;
}) {
  const { provider: rawProvider, collector: rawCollector } = use(params);
      ...
      {artifact.isPending ? (
        ...
        <>
          <DocPageHeader
            ...
            description={data.description || "Collector reference documentation."}
            actions={
              acquirePlatform ? (
                <Link
                  href={acquireHref({
                      ...
                      <div className="rounded-md border border-border bg-surface-2 p-3">
                        ...
                        <ul className="mt-2 space-y-1">
                          {iamPolicies.map((policy) => (
                            <li key={policy.path} className="mono text-xs text-fg-subtle">
                              {policy.path}
                            </li>
                          ))}
                        </ul>
