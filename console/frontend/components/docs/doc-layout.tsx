    ...
    <div className="flex min-h-full">
      <aside className="hidden w-56 shrink-0 border-r border-border bg-surface lg:block">
        <div className="sticky top-0 max-h-[calc(100vh-3.5rem)] overflow-y-auto p-4">
          <div className="sb-nav-section px-0">Providers</div>
          <nav className="space-y-3">
            {DOC_PROVIDERS.map((p) => {
              const providerActive = provider === p;
              return (
                <div key={p}>
                  <DocsNavLink
                    href={docsSectionHref(p, "authentication")}
                    active={providerActive && !section}
                    className={providerActive ? "text-fg" : undefined}
                  >
                  ...
                  {providerActive && (
                    <div className="mt-0.5 space-y-0.5">
                      {DOC_SECTIONS.map((s) => (
                        <DocsNavLink
                          key={s.id}
                          href={docsSectionHref(p, s.id)}
                          active={section === s.id && !collector}
                          indent
                        >
                          {docSectionLabel(s.id, p)}
                        </DocsNavLink>
                      ))}
                    </div>
                  )}
          ...
          {provider && section === "collectors" && (
            <>
              <div className="sb-nav-section mt-4 px-0">Collector index</div>
              {artifactsPending ? (
                <div className="py-6">
...
export function DocSection({
  ...
  return (
    <section id={id} className="scroll-mt-6">
      ...
      <div className="mt-2 space-y-2 text-sm leading-relaxed text-fg-subtle">{children}</div>
    </section>
  );
}
