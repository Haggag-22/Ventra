...
try:
    # Resolved from the installed distribution's metadata, which hatch-vcs (setuptools-scm)
    # derives from the git tag at build/install time. Recorded in every manifest as ``tool_version``, so an
