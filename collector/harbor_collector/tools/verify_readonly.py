...
Two modes:
  * ``python -m harbor_collector.tools.verify_readonly <policy.json>`` — check an IAM policy.
  * ``python -m harbor_collector.tools.verify_readonly --collectors`` — check that every
...
from ..common.base import assert_readonly
