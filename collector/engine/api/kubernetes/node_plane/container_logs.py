...
class ContainerLogsCollector(NodePlaneCollector):
    ...
    def collect(self) -> SourceResult:
        ...
        for log_path in node.iter_glob(f"{_PODS_LOG_ROOT}/*/*/*.log"):
            ...
            collected += 1
            files.append(wf)
            index.append({**meta, "host_path": host_path, "archive_path": wf.path, "sha256": wf.sha256})

        files.append(self.write_json({"logs": index, "node": nc.to_dict()}, "config.json"))
        self.write_meta({"source": self.name, "logs": collected})
        ...
        return SourceResult(
            ...
            gaps=gaps,
            notes=f"{collected} container log file(s) from {node.hostname() or 'node'}.",
        )
