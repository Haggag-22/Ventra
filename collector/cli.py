...
def _add_kubernetes_parser(sub: argparse._SubParsersAction) -> None:
    k8s = sub.add_parser("kubernetes", help="Collect from an on-prem Kubernetes cluster.")
    k8s.add_argument("--case", help="Case identifier, e.g. CASE-2026-0042.")
    k8s.add_argument("--engagement", default="", help="Optional engagement/matter id.")
    k8s.add_argument(
        "--kubeconfig",
        default="",
        help="Path to kubeconfig (default: KUBECONFIG env or credentials/kubeconfig.yaml from the kit).",
    )
    k8s.add_argument(
        "--context",
        default="",
        help="Kubeconfig context name (default: VENTRA_K8S_CONTEXT / acquisition.yaml).",
    )
    k8s.add_argument(
        "--node-root",
        default="",
        help="Host filesystem root for node-plane collectors (default: VENTRA_NODE_ROOT or /).",
    )
    k8s.add_argument(
        "--node-name",
        default="",
        help="Node hostname stamped on evidence (default: VENTRA_NODE_NAME or hostname).",
    )
    k8s.add_argument("--regions", default="", help="Unused for Kubernetes (accepted for kit parity).")
    k8s.add_argument("--since", default=None, help="Window start (YYYY-MM-DD or RFC3339 UTC).")
    k8s.add_argument("--until", default=None, help="Window end (YYYY-MM-DD or RFC3339 UTC).")
    k8s.add_argument("--out", default="./ventra-evidence", help="Output directory for the package.")
    k8s.add_argument("--transport", default="local", help="local | s3-presigned:<url> | sftp:...")
    k8s.add_argument("--key", default=None, help="Signing key path for cosign/minisign.")
    k8s.add_argument(
        "--case-store",
        default=None,
        help="Case store for auto-ingest after collect (default: $VENTRA_CASE_STORE or ./cases).",
...
def _add_run_parser(sub: argparse._SubParsersAction) -> None:
    run = sub.add_parser(
        "run",
        help="Run a downloaded Collection Kit (.kit) offline.",
    )
    run.add_argument("kit", help="Path to the .kit file (or extracted kit directory).")
    run.add_argument(
        "--out",
        default="evidence",
...
def _add_import_parser(sub: argparse._SubParsersAction) -> None:
    imp = sub.add_parser(
        "import",
        help="Import a sealed evidence package (or ventra run output dir) into a case.",
    )
    imp.add_argument(
        "path",
        help="Sealed .tar.zst package, or an output directory containing one.",
    )
    imp.add_argument(
        "--into",
        default="",
        help="Case ID to import into (optional when the package manifest already has one).",
    )
    imp.add_argument(
        "--case-store",
        default="",
        help="Case store root (default: VENTRA_CASE_STORE or ./cases).",
    )
...
def _add_kit_parser(sub: argparse._SubParsersAction) -> None:
    ...
    build.add_argument("--out", default="kit.kit", help="Output path (.kit recommended; .zip for legacy).")
    build.add_argument(
        "--no-iam",
        action="store_true",
...
def _normalize_argv(argv: list[str]) -> list[str]:
    ...
    if argv[0] in ("--version", "-V", "-h", "--help"):
        return argv
    if argv[0] in ("collect", "dev", "gui", "artifacts", "kit", "run", "import"):
        return argv
    if argv[0] == "aws":
        return ["collect", *argv]
    if argv[0] == "azure":
...
def _cli_reporter(*, quiet: bool = False, json_mode: bool = False, cloud: str = "aws"):
    ...
    if cloud == "azure":
        ...
        from .engine.registry import KUBERNETES_REGISTRY as REGISTRY

        cloud_title = "Kubernetes"
    else:
...
def _run_kubernetes(args) -> int:
    import os
    import socket

    from .engine.api.kubernetes.runner import KubernetesRunConfig, run_kubernetes_collection
    from .engine.registry import collector_order_for_cloud, registry_for_cloud
    from .lib.transport import get_transport

    registry = registry_for_cloud("kubernetes")
    order = collector_order_for_cloud("kubernetes")

    if args.list_collectors:
        for name, cls in sorted(registry.all().items()):
            print(f"  {name:<28} {cls.description}")
        return 0
    if getattr(args, "list_packs", False):
        return _print_packs("kubernetes")

    try:
        collectors, artifact_refs, case_override, eng_override, spec = _plan_collection(
            args, "kubernetes", list(order), registry
        )
    except ValueError as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 2
    ...
    kubeconfig = (getattr(args, "kubeconfig", "") or os.environ.get("KUBECONFIG", "")).strip()
    if not kubeconfig and spec and spec.kubeconfig:
        candidate = (acq_dir / spec.kubeconfig).resolve()
        if candidate.is_file():
            kubeconfig = str(candidate)
    context = (
        (getattr(args, "context", "") or "").strip()
        or os.environ.get("VENTRA_K8S_CONTEXT", "").strip()
        or ((spec.k8s_context if spec else "") or "")
    )
    node_root = (
        (getattr(args, "node_root", "") or "").strip()
        or os.environ.get("VENTRA_NODE_ROOT", "").strip()
        or ((spec.node_root if spec else "") or "")
        or "/"
    )
    node_name = (
        (getattr(args, "node_name", "") or "").strip()
        or os.environ.get("VENTRA_NODE_NAME", "").strip()
        or socket.gethostname()
    )

    json_mode = args.json_output
    reporter, console = _cli_reporter(quiet=args.quiet, json_mode=json_mode, cloud="kubernetes")

    plan_label, artifact_labels, artifact_severities = _matrix_plan_fields(
        args, "kubernetes", collectors, artifact_refs
    )
    ...
    if json_mode:
        ...
        payload = {
            "case_id": case_id,
            "engagement_id": (args.engagement or eng_override) or None,
            "account_id": getattr(reporter, "_account", None),
            "collectors": reporter.collectors_report(),
            "coverage_gaps": reporter.coverage_gaps(),
            "package": {
                "path": str(package.path),
...
def _run_import(args) -> int:
    from .kit.import_cmd import ImportError_, import_evidence
    from .lib.ingest import default_case_store

    try:
        info = import_evidence(
            Path(args.path),
            into_case_id=(args.into or "").strip() or None,
            case_store=Path(args.case_store) if (args.case_store or "").strip() else default_case_store(),
        )
    ...
    print()
    print(f"Imported into case {info['case_id']}")
    print(f"  package:  {info['package']}")
    print(f"  case dir: {info['case_dir']}")
    print(f"  events:   {info['events']}")
    print(f"  integrity:{info['integrity']}")
    if info.get("sources_loaded"):
        print(f"  sources:  {', '.join(info['sources_loaded'])}")
