...
class KubernetesClientFactory:
    ...
    def _get_raw(self, action: str, path: str) -> Any:
        """GET a non-typed API path (``/version``, ``/apis``, beta resources) as plain JSON.

        The generated clients cover typed, still-served resources only. Discovery endpoints
        and removed-but-sometimes-present betas (PodSecurityPolicy) have no typed method, so
        they go through the raw client with the same error translation as everything else.
        """
        api = self._load()
        try:
            return api.call_api(
    ...
    def cluster_version(self) -> dict[str, Any]:
        """``GET /version`` — version, build date, platform (the cluster-info snapshot row)."""
        data = self._get_raw("get /version", "/version")
        return dict(data) if isinstance(data, dict) else {"_raw": data}

    def api_resources(self) -> dict[str, Any]:
        """``GET /apis`` + ``/api`` discovery, then the resource list for each group version.

        Reveals CRDs, aggregated APIs, and anything unusual installed. A group whose resource
        list cannot be read is recorded in ``errors`` rather than failing discovery outright.
        """
        out: dict[str, Any] = {"group_versions": [], "resources": {}, "errors": {}}

        core = self._get_raw("get /api", "/api")
        core_versions = list((core or {}).get("versions") or []) if isinstance(core, dict) else []
        paths: list[tuple[str, str]] = [(v, f"/api/{v}") for v in core_versions]

        groups_doc = self._get_raw("get /apis", "/apis")
        groups = list((groups_doc or {}).get("groups") or []) if isinstance(groups_doc, dict) else []
        out["groups"] = [
            {
                "name": g.get("name", ""),
                "preferred_version": (g.get("preferredVersion") or {}).get("groupVersion", ""),
                "versions": [v.get("groupVersion", "") for v in (g.get("versions") or [])],
            }
            for g in groups
            if isinstance(g, dict)
        ]
        for group in out["groups"]:
            gv = group["preferred_version"]
            if gv:
                paths.append((gv, f"/apis/{gv}"))

        for group_version, path in paths:
            out["group_versions"].append(group_version)
            try:
                doc = self._get_raw(f"get {path}", path)
            except (KubeAccessDenied, KubeNotFound) as exc:
                out["errors"][group_version] = exc.message
                continue
            except Exception as exc:  # noqa: BLE001 - one group must not break discovery
                out["errors"][group_version] = str(exc)
                continue
            resources = (doc or {}).get("resources") or [] if isinstance(doc, dict) else []
            out["resources"][group_version] = [
                {
                    "name": r.get("name", ""),
                    "kind": r.get("kind", ""),
    ...
    def can_i_url(self, verb: str, path: str) -> bool:
        """SelfSubjectAccessReview for a non-resource URL (``/version``, ``/apis``)."""
        from kubernetes import client  # noqa: PLC0415

        auth = self._client("AuthorizationV1Api")
        body = client.V1SelfSubjectAccessReview(
            spec=client.V1SelfSubjectAccessReviewSpec(
                non_resource_attributes=client.V1NonResourceAttributes(verb=verb, path=path)
            )
        )
        try:
            review = auth.create_self_subject_access_review(body)
    ...
    def list_events_v1(self) -> list[dict[str, Any]]:
        """``events.k8s.io/v1`` Events — the modern API group, served alongside ``core/v1``."""
        events = self._client("EventsV1Api")
        return self._list("list events.k8s.io events", events.list_event_for_all_namespaces)

    def list_persistent_volumes(self) -> list[dict[str, Any]]:
        core = self._client("CoreV1Api")
        return self._list("list persistentvolumes", core.list_persistent_volume)
