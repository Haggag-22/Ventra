            ...
            _ct(
                {
                    ...
                    "eventSource": rng.choice(
                        ["ec2.amazonaws.com", "s3.amazonaws.com", "sts.amazonaws.com", "iam.amazonaws.com"]
                    ),
            ...
            _ct(
                {
                    ...
                    "eventSource": (
                        "iam.amazonaws.com"
                        if name.startswith(("List", "Get")) and "Bucket" not in name
            ...
            _ct(
                {
                    ...
                    "userAgent": boto_ua,
                    "requestParameters": {
                        "bucketName": "client-prod-db-backups",
...
def build_sts() -> list[dict]:
        ...
        return _ct(
            {
                ...
                "responseElements": {
                    "assumedRoleUser": {"arn": role_arn.replace(":role/", ":assumed-role/") + "/sess"}
                },
            }
        )
    ...
    for i in range(12):
        events.append(
            assume(
                f"arn:aws:iam::{ACCOUNT}:user/ci-deploy",
                "ci-deploy",
                app_role,
                LEGIT_IP,
                (BASE - timedelta(days=1) + timedelta(hours=i)).strftime("%Y-%m-%dT%H:%M:%SZ"),
            )
        )
...
def build_elb_alb() -> list[dict]:
    ...
    def alb(when: str, ip: str, request: str, status: int, ua: str) -> dict:
        ...
        return {
            ...
            "_ventra_lb_name": "web-alb",
            "_ventra_lb_type": "application",
            "_ventra_s3_bucket": "client-lb-logs",
        }
    ...
    events.append(
        alb(_t(-1500), ATTACKER_IP, "POST https://shop.example.com:443/admin/login HTTP/1.1", 200, browser_ua)
    )
    ...
    for i in range(20):
        events.append(
            alb(
                _t(-3600 + i * 120),
                LEGIT_IP,
                "GET https://shop.example.com:443/products HTTP/1.1",
                200,
                browser_ua,
            )
        )
...
def build_route53_resolver() -> list[dict]:
    ...
    def query(when: str, name: str, rcode: str, answer: str | None) -> dict:
        return {
            ...
            "srcaddr": "10.0.1.5",
            "srcport": str(40000 + rng.randint(0, 9999)),
            "transport": "UDP",
            "srcids": {"instance": "i-0a1b2c3d4e5f6a7b8"},
        }

    events = []
    # C2 beacon resolving every ~5 minutes after compromise.
    for i in range(12):
        events.append(query(_t(1300 + i * 300), "cdn-sync.evil-c2.net", "NOERROR", "185.220.101.45"))
    # DGA fallback noise (NXDOMAIN burst).
    for i in range(8):
        events.append(query(_t(1320 + i * 15), f"x{rng.randrange(16**8):08x}.biz", "NXDOMAIN", None))
    ...
    for i in range(3):
        ...
        events.append(query(_t(1340 + i * 12), f"{label}.tunnel.evil-c2.net", "NOERROR", "185.220.101.45"))
    # Baseline legit lookups.
    for i in range(15):
        events.append(query(_t(-3600 + i * 240), "api.payments-partner.com", "NOERROR", "52.94.236.10"))
    return events
...
def build_s3_access() -> list[dict]:
    ...
    for i in range(15):
        out.append(
            line(
                ...
                "customer-db.sql.gz",
                200,
                rng.randint(45_000_000, 60_000_000),
                cli,
            )
        )
    ...
    for i in range(3):
        out.append(
            line(
                1700 + i * 4, ATTACKER_IP2, dbadmin, "REST.DELETE.OBJECT", f"backups/db-{i}.bak", 204, 0, cli
            )
        )
...
def build_waf() -> list[dict]:
    ...
    for i in range(10):
        out.append(sample(-3000 + i * 120, LEGIT_IP, "US", "GET", "/products", "ALLOW", "", browser))
    return out
...
def build_guardduty() -> list[dict]:
    ...
    return [
        finding(
            ...
            7.5,
            "Console login from a Tor exit node",
            ATTACKER_IP,
            _t(30),
        ),
        finding(
            ...
            5.0,
            "API calls from a known malicious IP",
            ATTACKER_IP,
            _t(300),
        ),
        finding(
            ...
            8.0,
            "Administrative policy attached to user",
            ATTACKER_IP,
            _t(800),
        ),
        finding(
            "Persistence:IAMUser/AnomalousBehavior", 7.0, "Anomalous IAM user creation", ATTACKER_IP, _t(960)
        ),
        finding(
        ...
        finding(
            ...
            8.0,
            "Anomalous S3 data retrieval volume",
            ATTACKER_IP2,
            _t(1500),
        ),
    ]
...
def build_vpc_flow() -> list[dict]:
    ...
    for i in range(8):
        ...
        out.append(
            {
                "_ventra_region": REGION,
                "_ventra_vpc_id": vpc_secondary,
                "timestamp": s * 1000,
                "message": f"2 {ACCOUNT} {eni} 10.0.3.10 10.0.3.20 52000 443 6 8 900 {s} {s + 8} ACCEPT OK",
            }
        )
...
def build_iam_snapshot() -> dict:
    ...
    return {
        "users": [
            ...
            {
                "UserName": "ci-deploy",
                "Arn": f"arn:aws:iam::{ACCOUNT}:user/ci-deploy",
                "CreateDate": "2023-01-15T08:00:00Z",
                "AttachedManagedPolicies": [{"PolicyName": "ReadOnlyAccess", "PolicyArn": readonly_arn}],
                "UserPolicyList": [],
                "GroupList": ["deployers"],
                "AccessKeys": [
                    {
                        "AccessKeyId": "AKIAEXAMPLECI001",
                        "Status": "Active",
                        "CreateDate": "2023-01-15T08:00:00Z",
                        "LastUsed": {},
                    }
...
def build_s3_inventory() -> dict:
    return {
        "buckets": [
            {
                "name": "client-prod-db-backups",
                "region": REGION,
                "creation_date": "2023-02-01",
                "logging": None,
                "_ventra_no_access_logging": True,
                "policy_status": {"IsPublic": False},
                "object_lock": None,
            },
            {
                "name": "client-prod-assets",
                "region": REGION,
                "creation_date": "2023-02-01",
...
def build_ec2_inventory() -> dict:
    return {
        "instances": [
            {
                "InstanceId": "i-0web001",
                "_ventra_region": REGION,
                "State": {"Name": "running"},
                "PrivateIpAddress": "10.0.1.20",
                "PublicIpAddress": "52.0.0.20",
...
def build_account_snapshot() -> dict:
    return {
        ...
        "operator_user_id": "AROAEXAMPLE:omar",
        "partition": "aws",
        "regions_in_scope": [REGION],
        "org_id": "o-exampleorg123",
    }
...
def generate(out_dir: Path, case_id: str = "CASE-2026-0042") -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="ventra-demo-") as tmp:
        ...
        manifest = Manifest(
            ...
            regions=[REGION],
            operator=Operator(
                principal_arn=f"arn:aws:sts::{ACCOUNT}:assumed-role/IR-Responder/omar",
                user_id="AROAEXAMPLE:omar",
                source_ip="100.64.0.10",
            ),
            started_at=_t(-10),
            completed_at=_t(2000),
            profile_name="all",
            host_environment="cloudshell",
            host_os="Amazon Linux 2023",
        ...
        src(
            ...
            [
                (
                    "events.jsonl.gz",
                    _write_gz_jsonl(sd / "cloudtrail/events.jsonl.gz", build_cloudtrail() + build_sts()),
                ),
                (
                    "config.json",
                    _write_json(
                        sd / "cloudtrail/config.json",
                        {
                            "trails": [
                                {
                                    "Name": "org-trail",
        ...
        src(
            "guardduty",
            [("events.jsonl.gz", _write_gz_jsonl(sd / "guardduty/events.jsonl.gz", build_guardduty()))],
            notes="6 findings.",
        )
        src(
            "vpc_flow",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "vpc_flow/events.jsonl.gz", build_vpc_flow())),
                (
                    "config.json",
                    _write_json(
                        sd / "vpc_flow/config.json",
                        {
                            "flow_logs": [
                                {
        ...
        src(
            "elb_alb",
            [("events.jsonl.gz", _write_gz_jsonl(sd / "elb_alb/events.jsonl.gz", build_elb_alb()))],
        ...
        src(
            ...
            [
                (
                    "events.jsonl.gz",
                    _write_gz_jsonl(sd / "route53_resolver/events.jsonl.gz", build_route53_resolver()),
                )
            ],
        ...
        src(
            "waf",
            [("events.jsonl.gz", _write_gz_jsonl(sd / "waf/events.jsonl.gz", build_waf()))],
            notes="WAF sampled requests — SQLi/XSS blocked from attacker IP.",
        )
        src(
            "s3_access",
            [("events.jsonl.gz", _write_gz_jsonl(sd / "s3_access/events.jsonl.gz", build_s3_access()))],
            notes="S3 server access logs — DB-dump exfil, anonymous reads, cover-up deletes.",
        )
        # Inventory sources
        src(
            "iam",
            [("snapshot.json", _write_json(sd / "iam/snapshot.json", build_iam_snapshot()))],
            notes="3 users, 2 roles.",
        )
        src(
            "s3",
            [("snapshot.json", _write_json(sd / "s3/snapshot.json", build_s3_inventory()))],
            notes="3 buckets, 1 public.",
        )
        src(
            "ec2",
            [("snapshot.json", _write_json(sd / "ec2/snapshot.json", build_ec2_inventory()))],
            notes="2 instances; 1 shared snapshot.",
        )
        src(
            "account",
            [("snapshot.json", _write_json(sd / "account/snapshot.json", build_account_snapshot()))],
        ...
        manifest.add_source_result(
            SourceResult(
                ...
                gaps=[
                    ...
                    ("inspector2", GapReason.SERVICE_NOT_ENABLED, "Inspector2 not enabled in scope."),
                ],
