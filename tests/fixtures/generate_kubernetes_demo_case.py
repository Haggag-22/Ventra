...
def build_apiserver_audit() -> list[dict]:
    ...
    events: list[dict] = [
        ...
        _audit(-3500, "get", "configmaps", namespace="prod", name="web-config",
               user="ops@ventra.example", ip=ADMIN_IP),
        ...
        _audit(64, "create", "pods", subresource="exec", namespace="prod",
               name="web-7d9f8b6c4-xk2lp"),
        # 3. Credential access — the SA token from inside the pod reads a Secret.
        _audit(96, "get", "secrets", namespace="prod", name="db-credentials",
               user=VICTIM_SA, groups=["system:serviceaccounts", "system:authenticated"]),
        _audit(104, "get", "secrets", namespace="prod", name="registry-pull",
               user=VICTIM_SA, groups=["system:serviceaccounts", "system:authenticated"]),
        # 4. Privilege escalation.
        _audit(150, "create", "clusterrolebindings", name="svc-monitor-admin",
               request_object=crb),
        # 5. Execution — the privileged miner pod.
        _audit(190, "create", "pods", namespace="kube-system", name="kube-proxy-metrics",
               request_object=miner_spec),
        # 6. Port-forward for the C2 channel.
        _audit(240, "create", "pods", subresource="portforward", namespace="kube-system",
        ...
        _audit(330, "delete", "validatingwebhookconfigurations", name="gatekeeper-validating",
               code=403),
        # 9. Anti-forensics: delete burst against Events.
        *[
            _audit(400 + i * 3, "delete", "events", namespace="prod", name=f"evt-{i}")
            for i in range(14)
        ],
        # 10. Anonymous probe of the API.
        _audit(500, "get", "nodes", user="system:anonymous",
               groups=["system:unauthenticated"], code=403),
    ]
    return events
...
def build_events() -> list[dict]:
    def core(uid, offset, reason, kind, ns, name, message, etype="Warning", node=WORKER_NODE):
        return {
            "metadata": {"uid": uid, "name": f"{name}.{uid}", "namespace": ns,
    ...
    events = [
        core("ev-01", 195, "Pulling", "Pod", "kube-system", "kube-proxy-metrics",
             f"Pulling image \"{MINER_IMAGE}\"", etype="Normal"),
        core("ev-02", 203, "Pulled", "Pod", "kube-system", "kube-proxy-metrics",
             f"Successfully pulled image \"{MINER_IMAGE}\" in 7.6s", etype="Normal"),
        core("ev-03", 206, "Created", "Pod", "kube-system", "kube-proxy-metrics",
             "Created container metrics", etype="Normal"),
        core("ev-04", 520, "OOMKilling", "Pod", "prod", "web-7d9f8b6c4-xk2lp",
             "Memory cgroup out of memory: Killed process 4211 (node)"),
        core("ev-05", 560, "BackOff", "Pod", "prod", "web-7d9f8b6c4-xk2lp",
             "Back-off restarting failed container"),
        core("ev-06", 610, "FailedMount", "Pod", "kube-system", "kube-proxy-metrics",
             "MountVolume.SetUp failed for volume \"host\" : hostPath type check failed"),
        core("ev-07", 640, "Evicted", "Pod", "prod", "api-5f6c7d8e9-mn3kq",
    ...
    events.append(
        {
            "metadata": {"uid": "ev-08", "name": "gatekeeper-audit.ev-08",
                         "namespace": "gatekeeper-system", "creation_timestamp": _t(660)},
            "regarding": {"kind": "Pod", "namespace": "gatekeeper-system",
...
def _obj(kind: str, name: str, *, namespace: str = "", created: int = -86400, **extra) -> dict:
    meta: dict = {"name": name, "uid": f"uid-{kind}-{name}",
                  "creation_timestamp": _t(created)}
    if namespace:
...
def build_cluster_state() -> dict[str, list[dict]]:
    miner = _obj(
        "pods", "kube-proxy-metrics", namespace="kube-system", created=190,
        spec={
            ...
            "containers": [
                {"name": "metrics", "image": MINER_IMAGE,
                 "security_context": {"privileged": True}}
            ],
            "volumes": [{"name": "host", "host_path": {"path": "/"}}],
        },
        status={"phase": "Running",
                "container_statuses": [{"name": "metrics", "image": MINER_IMAGE,
                                        "image_id": "sha256:6b1f9c2e" + "0" * 56}]},
    )
    # The collector stamps its verdict onto the record before sealing.
    miner["_ventra_suspicious"] = [
        "hostNetwork", "hostPID", "hostPath!:/", "privileged:metrics",
        f"untrusted_image:{MINER_IMAGE}",
    ]

    web = _obj(
        "pods", "web-7d9f8b6c4-xk2lp", namespace="prod",
        spec={"node_name": WORKER_NODE, "service_account_name": "web-runner",
              "containers": [{"name": "web", "image": "registry.k8s.io/nginx:1.25"}]},
        status={"phase": "Running",
                "container_statuses": [{"name": "web", "image": "registry.k8s.io/nginx:1.25",
                                        "image_id": "sha256:aa11bb22" + "0" * 56}]},
    )
    gatekeeper = _obj(
        "pods", "gatekeeper-audit-6d4b9c8f5-t2wqz", namespace="gatekeeper-system",
        spec={"node_name": WORKER_NODE, "service_account_name": "gatekeeper-admin",
    ...
    return {
        ...
        "nodes": [
            _obj("nodes", CP_NODE, status={"node_info": {
                "kubelet_version": "v1.30.2", "os_image": "Ubuntu 22.04.4 LTS",
                "kernel_version": "5.15.0-107-generic",
                "container_runtime_version": "containerd://1.7.16"}}),
            _obj("nodes", WORKER_NODE, status={"node_info": {
                "kubelet_version": "v1.30.2", "os_image": "Ubuntu 22.04.4 LTS",
                "kernel_version": "5.15.0-107-generic",
                "container_runtime_version": "containerd://1.7.16"}}),
        ],
        ...
        "serviceaccounts": [
            _obj("serviceaccounts", "web-runner", namespace="prod",
                 secrets=[{"name": "web-runner-token"}]),
            _obj("serviceaccounts", "default", namespace="kube-system"),
            _obj("serviceaccounts", "gatekeeper-admin", namespace="gatekeeper-system",
                 automount_service_account_token=False),
        ],
        "secrets": [
            _obj("secrets", "db-credentials", namespace="prod", type="Opaque"),
            _obj("secrets", "registry-pull", namespace="prod",
                 type="kubernetes.io/dockerconfigjson"),
        ],
        "deployments": [
            _obj("deployments", "web", namespace="prod",
                 spec={"replicas": 3}, status={"ready_replicas": 2}),
        ],
        "cronjobs": [
            _obj("cronjobs", "backup", namespace="prod", spec={"schedule": "0 2 * * *"}),
            _obj("cronjobs", "sync-metrics", namespace="kube-system", created=210,
                 spec={"schedule": "*/5 * * * *"}),
        ],
        "persistentvolumes": [
            _obj("persistentvolumes", "pv-local-01",
                 spec={"host_path": {"path": "/mnt/data"}, "storage_class_name": "local"}),
            _obj("persistentvolumes", "pv-nfs-01",
        ...
        "mutatingwebhookconfigurations": [
            _obj("mutatingwebhookconfigurations", "gatekeeper-mutating-webhook-configuration",
                 webhooks=[{"name": "mutation.gatekeeper.sh",
                            "client_config": {"service": {"namespace": "gatekeeper-system",
                                                          "name": "gatekeeper-webhook-service"}}}]),
            _obj("mutatingwebhookconfigurations", "sidecar-injector", created=280,
                 webhooks=[{"name": "inject.evil.example",
                            "client_config": {"url": "https://185.220.101.45:8443/mutate"}}]),
        ],
        "validatingwebhookconfigurations": [
            _obj("validatingwebhookconfigurations", "gatekeeper-validating",
...
def build_cluster_state_sidecars(objects: dict[str, list[dict]]) -> dict[str, object]:
    ...
    suspicious = [
        ...
        for p in pods if p.get("_ventra_suspicious")
    ]
    images = []
    for pod in pods:
        for c in (pod.get("spec") or {}).get("containers") or []:
            ids = {cs.get("name"): cs.get("image_id", "")
    ...
    return {
        ...
        "service_accounts": {
            "accounts": [
                {"namespace": "prod", "name": "web-runner",
                 "automount_service_account_token": True, "automount_explicit": False,
                 "secrets": ["web-runner-token"], "image_pull_secrets": [],
                 "pods": ["web-7d9f8b6c4-xk2lp"]},
                {"namespace": "kube-system", "name": "default",
                 "automount_service_account_token": True, "automount_explicit": False,
                 "secrets": [], "image_pull_secrets": [], "pods": ["kube-proxy-metrics"]},
                {"namespace": "gatekeeper-system", "name": "gatekeeper-admin",
                 "automount_service_account_token": False, "automount_explicit": True,
        ...
        "hostpath_volumes": [
            {"name": "pv-local-01", "path": "/mnt/data", "storageClassName": "local",
             "claimRef": {}},
        ],
        "environment": {
            "cluster": {"determined": True, "gitVersion": "v1.30.2", "platform": "linux/amd64",
                        "buildDate": "2026-05-14T09:12:33Z", "major": "1", "minor": "30"},
            "discovery": {"determined": True,
                          "group_versions": ["v1", "apps/v1", "batch/v1",
                                             "rbac.authorization.k8s.io/v1",
                                             "templates.gatekeeper.sh/v1"],
                          "groups": [], "resources": {}, "errors": {}},
        },
    }
...
def build_rbac() -> dict[str, list[dict]]:
    cluster_admin = _obj("clusterroles", "cluster-admin",
                         rules=[{"verbs": ["*"], "resources": ["*"], "api_groups": ["*"]}])
    cluster_admin["_ventra_grants"] = [
        "cluster-admin", "can read secrets cluster-wide",
        "wildcard verb+resource (cluster-admin-equivalent)",
    ]
    view = _obj("clusterroles", "view",
                rules=[{"verbs": ["get", "list"], "resources": ["pods"], "api_groups": [""]}])
    secret_reader = _obj("roles", "secret-reader", namespace="prod",
                         rules=[{"verbs": ["get", "list"], "resources": ["secrets"],
                                 "api_groups": [""]}])
    secret_reader["_ventra_grants"] = ["can read secrets cluster-wide"]

    escalation = _obj("clusterrolebindings", "svc-monitor-admin", created=150,
                      role_ref={"api_group": "rbac.authorization.k8s.io",
                                "kind": "ClusterRole", "name": "cluster-admin"},
                      subjects=[{"kind": "ServiceAccount", "name": "web-runner",
                                 "namespace": "prod"}])
    escalation["_ventra_grants"] = cluster_admin["_ventra_grants"]

    anonymous = _obj("clusterrolebindings", "system-anon-reader", created=300,
                     role_ref={"api_group": "rbac.authorization.k8s.io",
                               "kind": "ClusterRole", "name": "cluster-admin"},
    ...
    benign = _obj("clusterrolebindings", "ops-view",
                  role_ref={"api_group": "rbac.authorization.k8s.io",
                            "kind": "ClusterRole", "name": "view"},
                  subjects=[{"kind": "Group", "name": "ops"}])

    ns_binding = _obj("rolebindings", "prod-secret-reader", namespace="prod",
                      role_ref={"api_group": "rbac.authorization.k8s.io",
...
# ---------------------------------------------------------------------------------------
def _journal(offset: int, unit: str, message: str, *, priority: int = 6,
             node: str = WORKER_NODE) -> dict:
    return {
...
def build_kubelet_logs() -> list[dict]:
    return [
        _journal(191, "kubelet.service",
                 'SyncLoop ADD "kube-system/kube-proxy-metrics"'),
        _journal(193, "kubelet.service",
                 f'Pulling image "{MINER_IMAGE}" for container metrics'),
        _journal(205, "kubelet.service",
                 "Started container metrics in pod kube-system/kube-proxy-metrics"),
        _journal(207, "kubelet.service",
                 'Pod "kube-proxy-metrics" admitted with hostPID=true hostNetwork=true',
                 priority=4),
        _journal(521, "kubelet.service",
                 "Memory cgroup out of memory: OOM-killed process in prod/web", priority=3),
        _journal(612, "kubelet.service",
                 'MountVolume.SetUp failed for volume "host": permission denied', priority=3),
    ]


def build_runtime_logs() -> list[dict]:
    journal = [
        _journal(193, "containerd.service",
                 f'PullImage "{MINER_IMAGE}" from registry docker.io'),
        _journal(204, "containerd.service",
                 f'ImageCreate event name:"{MINER_IMAGE}"'),
        _journal(206, "containerd.service",
                 "StartContainer for 9f2c1e7b4a55 returns successfully"),
        _journal(700, "containerd.service",
                 "failed to pull image \"docker.io/evilcorp/stage2:latest\": "
                 "unauthorized: authentication required", priority=3),
    ]
    inspect = [
        {
            ...
            "container": "metrics",
            "inspect": {
                "status": {"id": "9f2c1e7b4a5561d3", "state": "CONTAINER_RUNNING",
                           "image": {"image": MINER_IMAGE}},
                "info": {"runtimeSpec": {"process": {"args": ["/xmrig", "--donate-level=1"]}},
...
def build_etcd_logs() -> list[dict]:
    return [
        _journal(150, "etcd.service",
                 "applied a new ClusterRoleBinding write, revision 44120", node=CP_NODE),
        _journal(660, "etcd.service",
                 "rejected connection from 10.20.4.9:51022 (error \"tls: bad certificate\")",
                 priority=3, node=CP_NODE),
    ]
...
def generate(out_dir: Path, case_id: str = "CASE-2026-K8S1") -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="ventra-k8s-demo-") as tmp:
        ...
        manifest = Manifest(
            ...
            regions=[],
            operator=Operator(principal_arn="kubernetes:ventra-collector",
                              user_id="ventra-collector", source_ip=ADMIN_IP),
            started_at=_t(-60), completed_at=_t(900),
            profile_name="all",
            host_environment="local", host_os="Ubuntu 22.04.4 LTS",
        ...
        src("k8s_events", [
            ...
            ("config.json", _write_json(sd / "k8s_events/config.json", {
                "event_ttl": "1h0m0s",
                "event_ttl_source": "kube-apiserver manifest (--event-ttl=1h0m0s)",
                "collected": len(events), "flagged": 5,
                "apis": {"core/v1": {"readable": True, "events": 7},
        ...
        src("k8s_container_logs", [
            ...
            ("config.json", _write_json(sd / "k8s_container_logs/config.json", {
                "logs": [
                    {"namespace": "kube-system", "pod": "kube-proxy-metrics", "uid": "uid9",
                     "container": "metrics",
                     "host_path": "/var/log/pods/kube-system_kube-proxy-metrics_uid9/metrics/0.log",
                     "rotated": False, "sha256": "0" * 64},
                    {"namespace": "prod", "pod": "web-7d9f8b6c4-xk2lp", "uid": "uid1",
                     "container": "web",
                     "host_path": "/var/log/pods/prod_web-7d9f8b6c4-xk2lp_uid1/web/0.log.20260611",
                     "rotated": True, "sha256": "0" * 64},
                ],
                "rotated_logs": 1,
        ...
        src("k8s_runtime_logs", [
            ("runtime.jsonl.gz", _write_gz_jsonl(sd / "k8s_runtime_logs/runtime.jsonl.gz",
                                                 runtime[:4])),
            ("container_inspect.jsonl.gz", _write_gz_jsonl(
                sd / "k8s_runtime_logs/container_inspect.jsonl.gz", runtime[4:])),
            ("live_state.json", _write_json(sd / "k8s_runtime_logs/live_state.json", {
                "available": True,
                "containers": {"containers": [{"id": "9f2c1e7b4a5561d3",
                                               "labels": {"io.kubernetes.pod.name":
                                                          "kube-proxy-metrics"}}]},
            })),
            ("config.json", _write_json(sd / "k8s_runtime_logs/config.json", {
                "runtime": {"runtime": "containerd", "version": "1.7.16",
                            "socket": "/run/containerd/containerd.sock",
                            "storage_root": "/var/lib/containerd",
                            "detected_from": "socket"},
                "unit": "containerd", "source": "journalctl -u containerd",
        ...
        src("k8s_kubelet_logs", [
            ("kubelet.jsonl.gz", _write_gz_jsonl(sd / "k8s_kubelet_logs/kubelet.jsonl.gz",
                                                 kubelet)),
            ("config.json", _write_json(sd / "k8s_kubelet_logs/config.json", {
        ...
        src("k8s_etcd", [
            ...
            ("config.json", _write_json(sd / "k8s_etcd/config.json", {
                "posture": {
                    "determined": True,
                    ...
                    "encryption_at_rest": {"determined": True, "enabled": False,
                                           "config_file": ""},
                },
                "data_dir": {"path": "/var/lib/etcd", "present": True, "mode": "0o700",
                             "total_bytes": 41943040, "files": []},
                "tls_material": {"dir": "/etc/kubernetes/pki/etcd", "present": True,
                                 "files": [
                                     {"path": "/etc/kubernetes/pki/etcd/server.crt",
                                      "captured": True, "sha256": "3" * 64, "mode": "0o644"},
                                     {"path": "/etc/kubernetes/pki/etcd/server.key",
                                      "captured": False, "sha256": "4" * 64, "mode": "0o600",
                                      "note": "private key — hashed only, contents not captured"},
                                 ]},
                "topology": {"available": True, "member_count": 1,
                             "unhealthy_endpoints": [],
                             "endpoints": "https://127.0.0.1:2379"},
                "log_files": 0, "db_dumped": False,
                "node": {"node_name": CP_NODE, "runtime": "containerd"},
            })),
        ], status=SourceStatus.PARTIAL,
            gaps=[("k8s_etcd", GapReason.LOGGING_NOT_CONFIGURED,
                   "Kubernetes encryption-at-rest is NOT configured — every Secret in the "
                   "cluster is stored in etcd in plaintext.")],
            notes="etcd posture: 1 issue; 1 member; db_dumped=False.")

        # --- Audit log + posture --------------------------------------------------------
        src("k8s_apiserver_audit", [
            ("events.jsonl.gz", _write_gz_jsonl(sd / "k8s_apiserver_audit/events.jsonl.gz",
                                                audit)),
            ("audit.log", _write_text(sd / "k8s_apiserver_audit/audit.log",
                                      "\n".join(json.dumps(a) for a in audit) + "\n")),
            ("detections.json", _write_json(sd / "k8s_apiserver_audit/detections.json", {
                "exec_or_attach": [{"auditID": "aud-00064", "user": ATTACKER_USER}],
                "secret_read": [{"auditID": "aud-00096", "user": VICTIM_SA},
                                {"auditID": "aud-00104", "user": VICTIM_SA}],
                "rbac_change": [{"auditID": "aud-00150", "user": ATTACKER_USER},
            ...
            ("config.json", _write_json(sd / "k8s_apiserver_audit/config.json", {
                "audit_path": "/var/log/kubernetes/audit/audit.log",
                "audit_path_source":
                    "kube-apiserver manifest (--audit-log-path=/var/log/kubernetes/audit/audit.log)",
                "audit_webhook_config_file": "",
                "files": ["/var/log/kubernetes/audit/audit.log"],
                "records": len(audit), "parse_errors": 0,
                "node": {"node_name": CP_NODE, "runtime": "containerd"},
                "detection_counts": {"exec_or_attach": 1, "secret_read": 2, "rbac_change": 2,
                                     "privileged_pod_create": 1, "portforward": 1,
                                     "anonymous_subject": 1, "delete_burst": 1,
                                     "unexpected_source_ip": 1},
            })),
        ], notes=f"{len(audit)} audit event(s); 10 detection hit(s).")

        src("k8s_audit_posture", [
            ("config.json", _write_json(sd / "k8s_audit_posture/config.json", {
                "audit_enabled": True, "log_backend": True,
                "flags": {"audit-log-path": "/var/log/kubernetes/audit/audit.log",
                          "audit-policy-file": "/etc/kubernetes/audit-policy.yaml",
                          "audit-log-maxage": "7", "audit-log-maxbackup": "3",
                          "audit-log-maxsize": "100"},
                "audit_policy_file": "/etc/kubernetes/audit-policy.yaml",
                "audit_policy": {"rules": [{"level": "Metadata",
                                            "resources": [{"group": "",
                                                           "resources": ["secrets"]}]}]},
                "policy_weaknesses": [
                ...
                "rotation": {"audit-log-maxage": 7, "audit-log-maxbackup": 3,
                             "audit-log-maxsize": 100, "age_limited": True,
                             "backup_limited": True,
                             "issues": ["--audit-log-maxage=7 day(s): audit history older "
                                        "than that has already been deleted."]},
            })),
        ], status=SourceStatus.PARTIAL,
            gaps=[("k8s_audit_posture", GapReason.LOGGING_NOT_CONFIGURED,
        ...
        for kind, rows in objects.items():
            state_files.append((f"{kind}.jsonl.gz", _write_gz_jsonl(
                sd / f"k8s_cluster_state/{kind}.jsonl.gz", rows)))
        for stem, payload in sidecars.items():
            state_files.append((f"{stem}.json", _write_json(
                sd / f"k8s_cluster_state/{stem}.json", payload)))
        state_files.append(("config.json", _write_json(
            sd / "k8s_cluster_state/config.json", {
                ...
                "mutating_webhooks": ["gatekeeper-mutating-webhook-configuration",
                                      "sidecar-injector"],
                ...
                "pod_security": {"namespaces_without_psa": 1, "psp_api_present": False,
                                 "psp_count": 0},
                "api_group_versions": 5,
            })))
        src("k8s_cluster_state", state_files, status=SourceStatus.PARTIAL,
            gaps=[
                ("k8s_cluster_state", GapReason.NOT_PRESENT,
        ...
        for kind, rows in rbac.items():
            rbac_files.append((f"{kind}.jsonl.gz", _write_gz_jsonl(
                sd / f"k8s_rbac/{kind}.jsonl.gz", rows)))
        rbac_files.append(("dangerous_bindings.json", _write_json(
        ...
        rbac_files.append(("anonymous_bindings.json", _write_json(
            sd / "k8s_rbac/anonymous_bindings.json", [
                {"binding": "system-anon-reader", "subject": "User/system:anonymous",
                 "role": "ClusterRole/cluster-admin"},
            ])))
        rbac_files.append(("config.json", _write_json(sd / "k8s_rbac/config.json", {
            "counts": {k: len(v) for k, v in rbac.items()},
            "dangerous_binding_count": 2, "anonymous_binding_count": 1,
        })))
        src("k8s_rbac", rbac_files, status=SourceStatus.PARTIAL,
            gaps=[
                ("k8s_rbac", GapReason.NOT_PRESENT,
                 "2 binding(s) grant escalation-capable permissions."),
                ("k8s_rbac", GapReason.NOT_PRESENT,
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Generate a synthetic on-prem Kubernetes Ventra demo package."
    )
