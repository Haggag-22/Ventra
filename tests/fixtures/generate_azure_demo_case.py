...
def _diag(when: str, resource_id: str, category: str, props: dict, **extra) -> dict:
    return {"time": when, "resourceId": resource_id, "category": category, "properties": props, **extra}
...
def build_entra_audit() -> list[dict]:
    ...
    def audit(
        when: str, action: str, target: str, target_type: str = "Application", user: str = VICTIM_USER
    ) -> dict:
        return {
...
def build_activity_log() -> list[dict]:
    ...
    def act(when: str, op: str, caller: str, ip: str, resource: str, status: str = "Succeeded") -> dict:
        return {
            "eventTimestamp": when,
    ...
    events = [
        ...
        act(_t(360), "Microsoft.Resources/subscriptions/resources/read", VICTIM_USER, ATTACKER_IP, SUB),
        ...
        act(
            ...
            VICTIM_USER,
            ATTACKER_IP,
            f"{STORAGE}/blobServices/default/containers",
        ),
        # AZT402 — elevated access toggle
        act(
            _t(1010),
            "Microsoft.Authorization/elevatedAccess/read",
            VICTIM_USER,
            ATTACKER_IP,
        ...
        act(
            _t(1015),
            "Microsoft.Authorization/elevatedAccess/write",
            VICTIM_USER,
            ATTACKER_IP,
            f"{SUB}/providers/Microsoft.Authorization/elevatedAccess",
        ),
        ...
        act(
            _t(1080), "Microsoft.Storage/storageAccounts/listKeys/action", VICTIM_USER, ATTACKER_IP2, STORAGE
        ),
        ...
        act(_t(1260), "Microsoft.Compute/virtualMachines/runCommand/action", VICTIM_USER, ATTACKER_IP2, VM),
        ...
        act(_t(1390), "Microsoft.Logic/workflows/triggers/list/action", VICTIM_USER, ATTACKER_IP2, LOGIC),
        # AZT504 — watcher task
        act(
            _t(1400),
        ...
        act(
            ...
            VICTIM_USER,
            ATTACKER_IP2,
            f"{NSG}/securityRules/AllowAttacker",
        ),
        ...
        act(
            ...
            VICTIM_USER,
            ATTACKER_IP2,
            f"{SUB}/providers/Microsoft.ManagedServices/registrationDefinitions",
        ),
        ...
        act(
            ...
            VICTIM_USER,
            ATTACKER_IP2,
            f"{SUB}/providers/Microsoft.Authorization/policyAssignments/evil",
        ),
        ...
        act(
            _t(1800),
            "Microsoft.Storage/storageAccounts/blobServices/generateUserDelegationSas/action",
            VICTIM_USER,
            ATTACKER_IP2,
            STORAGE,
        ),
        ...
        act(
            ...
            VICTIM_USER,
            ATTACKER_IP2,
            f"{STORAGE}/fileServices/default/shares/backups",
        ),
        # AZT703 — replication
        act(
            _t(1830),
        ...
        act(
            _t(1840),
            "Microsoft.KeyVault/vaults/deletedSecrets/recover/action",
            VICTIM_USER,
            ATTACKER_IP2,
            f"{KV}/deletedSecrets/db-password",
        ),
        act(
            _t(1850),
...
def _flow_record(
    src: str, dst: str, dport: int, action: str, nbytes: int, when: str, resource_id: str
) -> dict:
    return {
        "srcaddr": src,
        "dstaddr": dst,
...
def build_vnet_flow() -> list[dict]:
    ...
    for i in range(15):
        out.append(
            _flow_record(
                "10.0.2.4", "10.0.3.10", 443, "ALLOW", rng.randint(500, 8000), _t(800 + i * 30), VNET_FLOW
            )
        )
    ...
    for i, port in enumerate([22, 3389, 445, 1433, 5985, 8080, 8443]):
        out.append(
            _flow_record(
                ATTACKER_IP, "10.0.1.5", port, "DENY", rng.randint(40, 200), _t(200 + i * 15), VNET_FLOW
            )
        )
    # Large egress exfil
    for i in range(10):
        out.append(
            _flow_record(
                ...
                443,
                "ALLOW",
                rng.randint(5_000_000, 12_000_000),
                _t(1500 + i * 45),
                VNET_FLOW,
            )
        )
...
def build_nsg_flow() -> list[dict]:
    ...
    for i in range(12):
        out.append(
            _flow_record(
                "10.0.2.4", "10.0.3.10", 443, "ALLOW", rng.randint(500, 8000), _t(820 + i * 25), NSG_FLOW
            )
        )
    for i, port in enumerate([22, 3389, 445, 5985]):
        out.append(
            _flow_record(
                ATTACKER_IP, "10.0.1.5", port, "DENY", rng.randint(40, 200), _t(210 + i * 18), NSG_FLOW
            )
        )
    for i in range(6):
        out.append(
            _flow_record(
                ...
                443,
                "ALLOW",
                rng.randint(3_000_000, 8_000_000),
                _t(1520 + i * 40),
                NSG_FLOW,
            )
        )
...
def build_entra_directory_snapshot() -> dict:
    return {
        "users": [
            ...
            {
                "id": "u-admin",
                "userPrincipalName": "it.admin@contoso-demo.com",
                "displayName": "IT Admin",
                "accountEnabled": True,
            },
            {
                "id": "u-persist",
                "userPrincipalName": PERSIST_USER,
        ...
        "applications": [
            {
                "id": MALICIOUS_APP_ID,
                "displayName": MALICIOUS_APP,
                "appId": MALICIOUS_APP_ID,
                "createdDateTime": _t(700),
            },
            {"id": "app-legit-001", "displayName": "Contoso HR Portal", "appId": "app-legit-001"},
        ],
        "service_principals": [
            {
                "id": "sp-evil-666",
...
def build_rbac_snapshot() -> dict:
    return {
        ...
        "role_definitions": [
            ...
            {"roleName": "Owner", "roleType": "BuiltInRole", "_ventra_subscription_id": SUBSCRIPTION_ID},
            {
...
def build_diag_posture() -> dict:
    ...
    return {
        ...
        "dns": {
            ...
            "event_hub_only": 0,
            "no_routing": 0,
            "sample": ["contoso-demo.com:storage"],
        },
        "storage_access": {
            "resources_total": 1,
            "routed_to_storage": 1,
...
def generate(out_dir: Path, case_id: str = "CASE-2026-AZ42") -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="ventra-azure-demo-") as tmp:
        ...
        manifest = Manifest(
            ...
            account_alias=TENANT_NAME,
            partition="azure",
            org_id=TENANT_ID,
            regions=[REGION],
            operator=Operator(
                principal_arn="azure-sp:sp-ventra-collector-demo", user_id=TENANT_ID, source_ip="100.64.0.10"
            ),
            started_at=_t(-10),
            completed_at=_t(2500),
            profile_name="all",
        ...
        src(
            ...
            [
                (
                    "snapshot.json",
                    _write_json(sd / "subscription/snapshot.json", build_subscription_snapshot()),
                )
            ],
        ...
        src(
            "entra_signin",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "entra_signin/events.jsonl.gz", entra_signin)),
                (
                    "_meta.json",
                    _write_json(
                        sd / "entra_signin/_meta.json",
        ...
        src(
            ...
            [
                ...
                (
                    "_meta.json",
                    _write_json(
                        sd / "entra_audit/_meta.json", {"source": "entra_audit", "records": len(entra_audit)}
                    ),
                ),
            ],
        ...
        src(
            "activity_log",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "activity_log/events.jsonl.gz", activity_log)),
                (
                    "_meta.json",
                    _write_json(
                        sd / "activity_log/_meta.json",
        ...
        src(
            ...
            [
                ...
                (
                    "config.json",
                    _write_json(
                        sd / "defender/config.json",
                        {"subscriptions": [{"subscription_id": SUBSCRIPTION_ID, "alerts": len(defender)}]},
                    ),
                ),
            ],
        ...
        src(
            "unified_audit",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "unified_audit/events.jsonl.gz", unified_audit)),
                (
                    "_meta.json",
                    _write_json(
                        sd / "unified_audit/_meta.json",
        ...
        src(
            ...
            [
                (
                    "events.jsonl.gz",
                    _write_gz_jsonl(sd / "unified_audit_search/events.jsonl.gz", unified_audit_search),
                ),
                (
                    "_meta.json",
                    _write_json(
                        sd / "unified_audit_search/_meta.json",
                        {"source": "unified_audit_search", "records": len(unified_audit_search)},
                    ),
                ),
            ],
        ...
        src(
            ...
            [
                ...
                (
                    "_meta.json",
                    _write_json(
                        sd / "oauth_consent/_meta.json", {"source": "oauth_consent", "records": len(oauth)}
                    ),
                ),
            ],
        ...
        src(
            "storage_access",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "storage_access/events.jsonl.gz", storage_access)),
            ],
        ...
        src(
            "log_analytics",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "log_analytics/events.jsonl.gz", log_analytics)),
                (
                    "_meta.json",
                    _write_json(
                        sd / "log_analytics/_meta.json",
        ...
        src(
            "azure_firewall",
            [
                ("events.jsonl.gz", _write_gz_jsonl(sd / "azure_firewall/events.jsonl.gz", azure_firewall)),
            ],
        ...
        src(
            ...
            [
                (
                    "snapshot.json",
                    _write_json(sd / "entra_directory/snapshot.json", build_entra_directory_snapshot()),
                )
            ],
        ...
        src(
            "rbac",
            [("snapshot.json", _write_json(sd / "rbac/snapshot.json", build_rbac_snapshot()))],
        ...
        src(
            ...
            [
                (
                    "snapshot.json",
                    _write_json(sd / "resource_graph/snapshot.json", build_resource_graph_snapshot()),
                )
            ],
        ...
        src(
            "diag_posture",
            [("config.json", _write_json(sd / "diag_posture/config.json", build_diag_posture()))],
