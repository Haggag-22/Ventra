"""
EKS Logs Config Collector
Collects logging configuration for a cluster.
...
def run_eks_logs_config(args):
    """Collect EKS logging configuration."""
    cluster_name = args.cluster
    print(f"[+] EKS Logs Config Collector")
    ...
    try:
        ...
        print("[+] Getting cluster logging configuration...")
        try:
            cluster_response = eks_client.describe_cluster(name=cluster_name)
            cluster = cluster_response.get("cluster", {})
            logs_data["logging"] = cluster.get("logging", {})
            
            enabled_logs = []
            cluster_logging = logs_data["logging"].get("clusterLogging", [])
            ...
            if enabled_logs:
                print(f"    ✓ Logging enabled for: {', '.join(enabled_logs)}")
            else:
                print(f"    ⚠ No logging enabled")
        ...
        filename = f"eks_logs_config_{safe_name}.json"
        filepath = _save_json_file(output_dir, filename, logs_data)
        if filepath:
            print(f"\n[✓] Saved logs config → {filepath}\n")
