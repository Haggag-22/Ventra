"""
ALB (Application Load Balancer) Access Logs Collector
Collects access log configuration and events for ALBs.
"""
import os
import json
import boto3
...
def run_alb_access_logs(args):
    """Collect ALB access log configurations."""
    print(f"[+] ALB Access Logs Collector")
    ...
    try:
        print("[+] Describing ALBs...")
        paginator = elbv2_client.get_paginator("describe_load_balancers")
        all_load_balancers = []
        
        for page in paginator.paginate():
            for lb in page.get("LoadBalancers", []):
                # Filter for ALBs only (Type == 'application')
                if lb.get("Type") == "application":
                    all_load_balancers.append(lb)
        
        if not all_load_balancers:
            print("    ⚠ No ALBs found in this region")
            return
        
        print(f"    ✓ Found {len(all_load_balancers)} ALB(s)")
        ...
        for lb in all_load_balancers:
            ...
            try:
                ...
                alb_info["Attributes"] = attributes
                ...
                alb_info["Attributes"] = {}
                alb_info["AccessLogs"] = {"Enabled": False}
            
            alb_data.append(alb_info)
        
        # Save to file
        filename = "alb_access_logs.json"
        filepath = _save_json_file(output_dir, filename, {
            "load_balancers": alb_data,
            "total": len(alb_data)
        })
        
        if filepath:
            print(f"\n[✓] Saved ALB access log configurations → {filepath}\n")
