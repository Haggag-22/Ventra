...
def run_cloudwatch_log_group(args):
    ...
    try:
        ...
        # 3. Collect log events (if --hours omitted, collect all available)
        print("[+] Collecting log events..." + ("" if collect_all else f" (last {hours} hours)"))
        try:
            ...
            logs_data["events_truncated"] = truncated
            if truncated:
                print(f"    ✓ Collected {logs_data['total_events']} log event(s) (TRUNCATED)")
            else:
