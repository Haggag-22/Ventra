...
- **Python 3.11+**
- **pipx** (recommended for production / open-source installs) or **[uv](https://docs.astral.sh/uv/)**
...
- **Console:** Node.js 18+ (installed automatically by `ventra gui`); for a global install without a clone, `pipx install 'ventra[console]'`

### Install

AWS, Azure, GCP, and Kubernetes collectors ship in the base package.

**Production / IR workstation (recommended — pipx):**
...
**Upgrade after a new PyPI release:**
...
```bash
pipx install 'ventra==1.0.0'
pipx install --force ./ventra-1.0.0-py3-none-any.whl
```
...
Use the clone + `uv run` path while hacking on Ventra itself. End users and production hosts should use **pipx** against PyPI so `ventra` is on `PATH` without activating a project venv.
...
**Releasing to PyPI** (clients then run `pipx upgrade ventra`): tag the commit — version comes
