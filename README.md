...
**Console access.** The console runs on your own workstation and listens on `127.0.0.1` only;
there is no login. It refuses requests addressed to any other host name, so a web page open in
the same browser can't reach it through DNS rebinding. If you deliberately serve it under
another name (e.g. behind your own proxy), list that name in `VENTRA_ALLOWED_HOSTS`.
